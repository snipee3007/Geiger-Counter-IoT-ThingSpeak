...
const char* ssid = "DAT";
const char* password = "giabao4497";
...
const char* myAPIString = "CIHNA1814TQ9XV9X";
...
void loop() {
  ...
  {
    if ((millis() - startMillis)/(1000*60) - minHolder >= 1){
      ...
      f.close();
      if ((millis() - startMillis)/(1000 * 60 * 60) - hourHolder >= 1){
        int field1 = ThingSpeak.setField(1, minuteHits);
        int field2 = ThingSpeak.setField(2, float(hourHits / 60.0));
        int field3 = ThingSpeak.setField(3, float(uSv));
        if (field1 == 200 && field2 == 200 && field3 == 200){
          Serial.println("Set 3 fields successful.");
        }
        else{
          Serial.println("Problem setting fields. HTTP error code " + String(field1) + ", " + String(field2) + ", " + String(field3));
        }
        int x = ThingSpeak.writeFields(channelNumber, myAPIString);
        if(x == 200){
          Serial.println("Channel update 3 fields successful.");
        }
        else{
        ...
        int field1 = ThingSpeak.setField(1, minuteHits);
        int field3 = ThingSpeak.setField(3, float(uSv));
        if (field1 == 200 && field3 == 200){
          Serial.println("Set 2 fields successful.");
        }
        else{
          Serial.println("Problem setting fields. HTTP error code " + String(field1) + ", " + String(field3));
        }
        int x = ThingSpeak.writeFields(channelNumber, myAPIString);
        if(x == 200){
          Serial.println("Channel update 2 fields successful.");
        }
        else{
          Serial.println("Problem updating channel. HTTP error code " + String(x));
        }
  ...
  client.println("    google.charts.setOnLoadCallback(drawTable); ");
 ...
 ///////////////////
  ...
  // Draw Table
  client.println("function drawTable(){");
  ...
  client.println("];");
  client.println("var sum = 0;");
  client.println("var hours = 1;");
  client.println("data.forEach(item => {");
  client.println("  if (item){");
  client.println("    console.log(item[0], hours, sum);");
  client.println("    if (Math.floor(parseInt(item[0]) / 60) < hours){");
  client.println("      sum += item[1]");
  client.println("    }");
  client.println("    else { ");
  client.println("      document.querySelector('#table').insertAdjacentHTML('beforeend', `<tr> <td>${hours}</td> <td>${(sum / 60).toFixed(2)}</td></tr>`); sum = 0;++hours;");
  client.println("    }");
  client.println("  };");
  client.println("})};");
  ...
  client.println("<tr>");
  client.println("<th>Time Elapse (Hours)</th>");
  client.println("<th>Avg CPM</th>");
  client.println("</tr>");
